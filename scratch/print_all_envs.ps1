...
foreach ($p in $nodeProcesses) {
    try {
        $env = [ProcessEnvDebug]::GetEnvironmentVariables($p.Id)
        if ($env -ne $null -and $env.Count -gt 0) {
            Write-Output "PID: $($p.Id) (Count: $($env.Count))"
            foreach ($k in $env.Keys) {
                if ($k -like "*SUPABASE*" -or $k -like "*PORT*" -or $k -like "*VITE*") {
                    Write-Output "  $k = $($env[$k])"
                }
            }
        }
    } catch {
        Write-Output "  Failed to read PID $($p.Id): $($_.Exception.Message)"
    }
}
